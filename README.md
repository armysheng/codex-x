...
它不想做一个“大而全 AI 平台”。它先把几件日常刚需做好：给 Codex 一个能长期记住上下文的工作区，一条命令初始化，按需接入飞书消息入口，再提供两个明确的 skill：解锁插件 skill 和移动端访问 skill。
...
- 飞书桥接、解锁插件 skill、移动端访问 skill 都是明确入口，不污染默认工作区。
...
| Plugin Unlock Skill | `skills/codex-plugin-unlock-zhuji` | 解锁 Codex App 插件，并让模型请求继续走筑基 Provider |
| Mobile Access Skill | `skills/codex-remote-access` | 从手机或另一台设备访问本地 Codex 工作区 |
...
- 想解锁插件：安装解锁插件 skill。
- 想在手机上访问 Codex：安装移动端访问 skill。

## 解锁插件和移动端访问

根目录 `skills/` 现在只放两个明确能力，不把它包装成泛泛的“能力市场”。默认工作区不会预装这些 skill，避免一开始就碰登录态或网络暴露。

当前包含：

| 能力 | 目录 | 适用场景 | 边界 |
|---|---|---|---|
| 解锁插件 skill | `codex-plugin-unlock-zhuji` | 解锁 Codex App 插件，并让模型请求继续走筑基 Provider | 先备份 `auth.json/config.toml`，重启前给回滚命令 |
| 移动端访问 skill | `codex-remote-access` | 从手机、平板或另一台设备访问 Codex 工作区 | 优先飞书桥接或私有网络，不默认公开暴露 |
...
│   └── codex-remote-access/          # 移动端访问 skill
├── docs/
└── examples/
```
...
- `packages/` 放产品主链路。
- `skills/` 只放解锁插件 skill 和移动端访问 skill，不默认安装。
...
- 继续瘦身默认 workspace template，让解锁插件 skill 和移动端访问 skill 保持明确安装、明确触发。
